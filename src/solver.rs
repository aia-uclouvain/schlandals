    ...
    fn pwmc(&mut self, is_lds: bool) -> Solution {
        ...
        if !is_lds {
            ...
            loop {
                ...
                solution.print();
                if self.parameters.start.elapsed().as_secs() >= self.parameters.timeout || solution.has_converged(self.parameters.epsilon) || 2*self.parameters.start.elapsed().as_secs() - prev_time >= self.parameters.timeout {
                    self.statistics.print();
                    return solution;
                }
                prev_time = self.parameters.start.elapsed().as_secs();
                discrepancy += 1;
            }
        }
    }
