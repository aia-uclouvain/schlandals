...
use crate::{common::*, core::graph::DistributionIndex};
...
pub struct ApproximateLearner<'a, B: BranchingDecision, const S: bool> {
    pub dacs: Vec<Vec<Option<Dac>>>, // [nb_approx, nb_dac]
    ...
    branching: Branching,
    inputs: Vec<PathBuf>,
    solvers: Vec<QuietSearchSolver<'a, B>>,
}
...
impl <'a, B: BranchingDecision, const S: bool> ApproximateLearner<'a, B, S> {

    /// Creates a new learner from the given graphs.
    pub fn new(inputs: Vec<PathBuf>, expected_outputs:Vec<f64>, epsilon:f64, branching: Branching, timeout:u64, folderdac: Option<PathBuf>, read:bool, ratio_learn:f64) -> Self {
        if !read {
            ...
            for (i , input) in learner.inputs.iter().enumerate() {
                println!("Compiling {}", input.display());
                let mut state = StateManager::default();
                let mut propagator = Propagator::new(&mut state);
                let mut graph = parser::graph_from_ppidimacs(&input, &mut state);
                let mut component_extractor = ComponentExtractor::new(&graph, &mut state);
                propagator.init(graph.number_clauses());
                let mut branching_heuristic: Box<dyn BranchingDecision> = match learner.branching {
                ...
                let mut constrained_distributions: Vec<DistributionIndex> = vec![];
                for distribution in graph.distributions_iter() {
                    if graph[distribution].is_constrained(&state) {
                        constrained_distributions.push(distribution);
                    }
                }
                let distributions_len = constrained_distributions.len();
                let mut compiler: DACCompiler<'_, dyn BranchingDecision> = DACCompiler::new(graph, state, component_extractor, branching_heuristic.as_mut(), propagator, (ratio_learn*distributions_len as f64).floor() as usize);
                if let Some(dac) = compiler.compile(timeout) {
                    let mut state = StateManager::default();
    ...
    fn reset_dacs(&mut self) {
        for batch_i in 0..self.dacs.len() {
            for dac_i in 0..self.dacs[batch_i].len() {
                if self.dacs[batch_i][dac_i].is_none() {
                    continue;
                }
                for node_i in 0..self.dacs[batch_i][dac_i].as_ref().unwrap().nodes.len() {
                    match self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node_i].get_type() {
                        TypeNode::Sum => {
                            if self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node_i].get_propagation().is_empty() {
                                self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[node_i].set_value(0.0);
                            }
                            else {
                        ...
                        TypeNode::Product => {
                            if self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node_i].get_propagation().is_empty() {
                                self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[node_i].set_value(1.0);
                            }
                            else {
                                self.reset_approx_node(batch_i, dac_i, node_i);
                            }
                        ...
                        TypeNode::Distribution{d,v} => {
                            ...
                            self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[node_i].set_value(proba);
                        },
                    }
                    self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[node_i].set_path_value(f128!(1.0));
                }
            }
        }
    }
    ...
    fn reset_approx_node(&mut self, batch_i:usize, dac_i:usize, node_i:usize){
        if let Some(dac) = &mut self.dacs[batch_i][dac_i] {
            solver.add_to_propagation_stack(dac.nodes[node_i].get_propagation().clone());
            match solver.solve() {
                Ok(p) => dac.nodes[node_i].set_value(p.to_f64()), 
                Err(_) => {}, // TODO flag them to no recompute it all the time
            };
    ...
    pub fn evaluate(&mut self) -> Vec<Vec<f64>> {
        ...
        for (i, batch) in self.dacs.iter_mut().enumerate() {
            println!("Batch {}", i);
            let mut evals_batch: Vec<f64> = vec![];
            for (j, opt_dac) in batch.iter_mut().enumerate() {
                println!("DAC {}", j);
                if let Some(dac) = opt_dac {
                    evals_batch.push(dac.evaluate().to_f64());
                }
                else {
                    evals_batch.push(0.0);
                }
            }
    ...
    pub fn compute_gradients(&mut self, batch_i:usize, gradient_loss: Vec<f64>){
        println!("Computing gradients {}", batch_i);
        self.zero_grads();
        for dac_i in 0..self.dacs[batch_i].len() {
            if self.dacs[batch_i][dac_i].is_none() {
                continue;
            ...
            for node in (0..self.dacs[batch_i][dac_i].as_ref().unwrap().nodes.len()).map(NodeIndex).rev(){ 
                println!("Node {} is approx {} type {:?}", node.0, self.dacs[batch_i][dac_i].as_ref().unwrap().is_node_incomplete(node), self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_type());
                // Iterate on all nodes from the DAC, top-down way
                let start = self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_input_start();
                let end = start + self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_number_inputs();
                let value = self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_value();
                let path_val = self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_path_value();
                // Update the path value for the children sum, product nodes 
                // and compute the gradient for the children leaf distributions
                for i in start..end {
                    println!("Child {}", i);
                    let child = self.dacs[batch_i][dac_i].as_ref().unwrap().get_input_at(i);
                    match self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[child.0].get_type() {
                        TypeNode::Sum => {
                            let mut val = path_val.clone() * &value;
                            val /= self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[child.0].get_value();
                            self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[child.0].set_path_value(val);
                        },
                        TypeNode::Product => {
                            self.dacs[batch_i][dac_i].as_mut().unwrap().nodes[child.0].set_path_value(path_val.clone());
                        },
                        TypeNode::Distribution{d,v} => {
                            // Compute the gradient for children that are leaf distributions
                            let mut factor = path_val.clone() * gradient_loss[dac_i];
                            if matches!(self.dacs[batch_i][dac_i].as_ref().unwrap().nodes[node.0].get_type(), TypeNode::Product) {
                                factor *= &value;
    ...
    pub fn train(&mut self, nepochs:usize, lr:f64, fout: Option<PathBuf>) {
        ...
        for e in 0..nepochs {
            ...
            let mut total_loss = vec![0.0; self.dacs[0].len()];
            for batch_i in 0..self.dacs.len() {
...
impl <'a, B: BranchingDecision, const S: bool> fmt::Display for ApproximateLearner<'a, B, S> {
...
impl <'a, B: BranchingDecision, const S: bool> ApproximateLearner<'a, B, S> {
    pub fn to_folder(learner: &ApproximateLearner<'a, B, S>, f: PathBuf) {
        ...
        for i in 0..learner.dacs.len() {
            for (j, opt_dac) in learner.dacs[i].iter().enumerate() {
                if let Some(dac) = opt_dac {
                    let mut outfile = File::create(f.join(format!("{}_{}.fdac", i,j))).unwrap();
                    match outfile.write(format!("{}evaluate {:.8}\n{}", dac, learner.expected_outputs[j], learner.inputs[j].display()).as_bytes()) {
                        Ok(_) => (),
    ...
    pub fn from_folder(f:PathBuf, branching: Branching, nb_approx: usize, epsilon:f64, inputs: Vec<PathBuf> ) -> ApproximateLearner<'a, B, S> {
